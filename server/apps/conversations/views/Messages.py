...
class MessageList(RetrieveAPIView):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        messages = Message.objects.filter(
            conversation=chat).order_by('created_at')
        serializer = self.get_serializer({'id': chat.id, 'title': chat.title, 'model': chat.model, 'prompt': chat.prompt, 'tokenLimit': chat.tokenLimit, 'maxLength': chat.maxLength,
                                         'temperature': chat.temperature, 'created_at': chat.created_at, 'updated_at': chat.updated_at, 'folder': chat.folder, 'messages': messages})
